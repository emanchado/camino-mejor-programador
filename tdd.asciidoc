...
Conclusión
----------
El desarrollo dirigido por pruebas nos ayuda a construir aplicaciones donde aquello que nos motiva a escribir el software guia cada línea de código, evitando a cada nivel desperdiciar nuestros esfuerzos en desarrollar funcionalidad que no sea exactamente la que necesitamos, o que sea simplemente innecesaria. La guía de las pruebas, combinada con el diseño continuo, hace posible un estilo de desarrollo orgánico, en el que el equipo evoluciona la aplicación, a medida que mejora su conocimiento de los requisitos del dominio y de la implementación ideal para resolverlos.

Si bien no hay un concenso absoluto en la idoineidad del desarrollo guiado por pruebas footnote:[Una crítica interesante es la que prefiere garantizar la corrección mediante el uso del lenguaje de programación, en lugar de probar sus casos de uso representativos, como hacemos en TDD footnote. Por ejemplo, en Java y lenguajes similares, a veces necesitamos hacer corresponder clases de una jerarquía origen con clases de una jerarquía destino. Si implementamos la correspondencia mediante una serie de instrucciones condicionales (+if+) o mediante una estructura de datos (+Map+), necesitaremos código para tratar cualquier tipo que no hayamos mencionado, probablemente lanzando una excepción, comportamiento que deberemos probar. En cambio, si implementamos la correspondencia mediante un método polimórfico en la jerarquía origen que sepa a qué clase corresponde en la jerarquía de destino, o si utilizamos expedición doble (_double dispatch_), el sistema de tipos hará imposible la ejecución del código que implementa la correspondencia con un tipo que no pertenezca a la jerarquía origen, por lo que no necesitaremos tratar con clases que no estén definidas en la correspondencia, ni probar dicho tratamiento.], su uso es cada vez más general, particularmente por equipos que desarrollan aplicaciones comerciales en lenguajes orientados a objetos. Además, la utilización de pruebas desde el primer momento, desvela el acoplamiento y nos incita a descomponer el código en unidades cohesivas con depencias claramente identificadas.
