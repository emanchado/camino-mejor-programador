...
Para terminar el paso rojo, ejecutamos la prueba, comprobando vemos que falla (lo que muchos entornos marcan en rojo) lanzando la excepción que esperábamos:
...
Trabajar dirigido por pruebas significa mantener siempre el correcto funcionamiento del sistema, con las pruebas satisfechas en todo momento; idealmente todas las versiones en el repositorio común deberán estar listas para ponerlas en explotación footnote:[De hecho, algunos equipos hacen exactamente eso, ponen cada versión que satisface la batería completa automáticamente en explotación] y la documentación proporcionada por las pruebas esté siempre al día. Para lograrlo, deberemos comprobar la satisfacción de las pruebas antes de enviar cualquier cambio al repositorio común; para mantener la factibilidad de esta tarea el equipo deberá invertir un esfuerzo continuo; las soluciones dependen del proyecto en sí, incluyendo a veces algunas de compromiso como relegar parte de las pruebas a un sistema de integración continua que las verificará cada vez que se detecte un cambio en el repositorio.
