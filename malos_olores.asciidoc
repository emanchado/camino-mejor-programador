...
El principio de sustitución de Liskov viene a decir de manera sutíl, que debemos considerar los objetos como cajas negras, sin conocer más que su interfaz pública. La forma en que típicamente incumplimos el principio es preguntando si el objeto que estamos manejando es, presuntamente, de un tipo concreto. Una vez que asumimos que es la implementación concreta X de la interfaz Y, le damos un trato exclusivo en función de las propiedades de X que no están en Y.
...
Cuando digo interfaz Y, me refiero indistintamente a una clase y a una "interface" típica de Java o C#. Toda clase es en sí misma una interfaz.
El problema con este enfoque es que quien consume el método "render" tiene que saber cómo funciona por dentro. Si le invoca pensando que funcionará con cualquier implementación de menú, se llevará una decepción, a pesar de que es lo que su firma dice. Nótese que este es un ejemplo de código sacado de contexto, tal vez en otro contexto el método "render" pertenece al menú en sí mismo.
Otra forma de romper el principio es usando conversión de tipos ("typecasts") porque de la misma manera preparan el terreno para consumir el objeto según sus detalles concretos. Una solución para poder generalizar tipos en clases que tienen dependencias, es el uso de genéricos (Java y C#). La generalización, al igual que la jerarquización, debe producirse como consecuencia de la eliminación de duplicidad:

        public abstract class FruitsController<Service>{
            Service service;

            public abstract string ServiceInvocation(string validInput);

	    public string FindByFarmer(string input){
                 input = input.ToLower(); // logica compleja y común ;-)                   
                 return ServiceInvocation(input);
            }
        }

        public class ApplesController<ApplesService>{
            public string ServiceInvocation(string validInput){
                 return service.FindApples(validInput);
            }
        }

        public class BananasController<BananasService>{
            public string ServiceInvocation(string validInput){
                 return service.FindBananas(validInput);
            }
        }
...
que sea. Me basta con preguntar si tiene tal o cual método. Algunos
...
- Conclusión
El olfato para los olores del código es desde mi punto de vista una cualidad del artesano de software. Hablamos de artesanía porque desconocemos todas las variables que lo convertirían en ingeniería.
