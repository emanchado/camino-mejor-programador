...
var coleccionComics = new BaseDatos('comics');
coleccionComics.alAgregar(function(comic) {
    console.log("Nuevo cómic añadido: " + comic.titulo);
});
// La siguiente línea debería imprimir «Nuevo cómic...» en la consola
coleccionComics.agregar({titulo: "Batman: The Dark Knight Returns",
                         autor:  "Frank Miller"});

// La implementación de alAgregar puede ser muy sencilla
function alAgregar(funcion) {
    this.funcionAgregar = funcion;
}
// La implementación de agregar también
function agregar(objeto) {
    this.datos.push(objeto);
    if (typeof(this.funcionAgregar) === 'function') {
        this.funcionAgregar(objeto);
    }
}
----------------------------------
